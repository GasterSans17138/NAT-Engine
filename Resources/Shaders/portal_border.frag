...
//  301's Fire Shader - Remix 3 by mu6k
...
void main()
{
	...
	float iTime = float(ubo.iTime);
    vec2 speed = vec2(0.1, 0.9);
    float shift = 1.327+sin(iTime*2.0)/2.4;
    
	float dist = 3.5-sin(iTime*0.4)/1.89;
    
    vec2 p = vec2(vOut.fragUV.x*10., vOut.fragUV.y);
    ...
    vec3 color = vec3(1.0/(pow(c+1.61,vec3(4.0))) * cos(shift * vOut.fragUV.y));
    
    color=ubo.matAmbient/(pow((r.y+r.y)* max(.0,p.y)+0.1, 4.0));;
    color = color/(1.0+max(vec3(0),color));
	if (length(color) < 0.6) discard;
	outColor = vec4(color.x, color.y, color.z, 1.0);
	outNormal = vec4(vOut.worldNormal, 1);
